...
const partnerPaldb = require("./reference/partner-skills/sources/paldb.json");
...
const NAV_SECTIONS = [
  ...
  },
  {
    ...
    label: "Tools",
    href: "pals.html",
    activeOn: ["pals", "partner-skills", "base-tips", "status-effects"],
    items: [
...
function renderFooter() {
  ...
  return (
    ...
    '<p>Palworld tools — work suitability, partner skills, and guides. Not affiliated with Pocketpair.</p>' +
    "</div>" +
    '<div class="footer-cols">' +
    cols +
    "</div></div>" +
    '<div class="footer-bottom">' +
    '<span>Game data source of truth: <a href="https://paldb.cc" target="_blank" rel="noopener noreferrer">paldb.cc</a></span>' +
    "</div></footer>"
  );
}
...
function shell({ title, subtitle, activeNav, body, headExtra = "", bodyScripts = "", showPromo = false }) {
  const promo = showPromo
    ? `<div class="promo-bar"><strong>Palhead</strong> — Palworld tools and database, powered by <a href="https://paldb.cc" target="_blank" rel="noopener noreferrer">paldb.cc</a>. <a href="pals.html">Open the pals database →</a></div>`
    : "";
...
function buildPartnerSkillsPayload() {
  ...
  const skills = (partnerPaldb.skills || []).map((s) => {
  ...
  return {
    builtAt: partnerPaldb.provenance?.scrapedAt || null,
    sourceUrl: partnerPaldb.provenance?.sourceUrl || "https://paldb.cc/en/Partner_Skill",
    count: skills.length,
    skills,
  };
}
    ...
    <main class="flex-1 page-wrap page-wrap-wide flex flex-col gap-3">
      <div class="page-title-bar flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p>
            Catalog from
            <a class="text-pal-text underline underline-offset-2 hover:text-white" href="https://paldb.cc/en/Partner_Skill" target="_blank" rel="noopener noreferrer">paldb.cc</a>
            — our source of truth for game data.
          </p>
        </div>
        <div class="text-xs text-pal-muted text-right space-y-0.5">
          <div><span id="builtAt" class="text-pal-text">—</span></div>
          <div><a class="text-pal-text underline underline-offset-2 hover:text-white" href="https://paldb.cc/en/Partner_Skill" target="_blank" rel="noopener noreferrer">View on paldb.cc</a></div>
        </div>
      </div>
      <section class="bg-pal-panel border border-pal-border rounded-lg p-3 md:p-4 space-y-3">
        <div class="flex flex-wrap items-end gap-3">
        ...
        <div class="flex flex-wrap items-center justify-between gap-2 text-sm pt-1 border-t border-pal-border/60">
          ...
          <div class="text-xs text-pal-muted">Click column headers to sort.</div>
        </div>
      </section>
      <section class="bg-pal-panel border border-pal-border rounded-lg overflow-hidden flex-1">
        <div class="table-wrap overflow-auto">
...
function filteredSorted() {
  ...
  let list = DATA.skills.filter(s => {
    if (!q) return true;
    const hay = [s.pal, s.name, s.description, s.type, s.no].join(' ').toLowerCase();
  ...
  tbody.innerHTML = list.map(s => {
    ...
    return '<tr class="border-t border-pal-border/40 align-top">' +
      ...
      '<td class="sticky-name px-2 py-2">' +
        ...
        '</div>' +
      '</td>' +
      '<td class="px-2 py-2 font-semibold whitespace-nowrap">' + escapeHtml(s.name || '—') + '</td>' +
      '<td class="px-2 py-2 text-xs text-pal-muted whitespace-nowrap">' + escapeHtml(s.type || '—') + '</td>' +
      '<td class="px-2 py-2 text-sm desc-cell">' + escapeHtml(s.description || '—') + '</td>' +
    '</tr>';
  }).join('') ||
    '<tr><td colspan="5" class="px-4 py-10 text-center text-pal-muted">No skills match your search.</td></tr>';
  renderHead();
}
...
document.getElementById('builtAt').textContent = DATA.builtAt
  ? 'Scraped ' + new Date(DATA.builtAt).toLocaleString()
  : 'paldb.cc snapshot';
bind();
renderTable();
</script>
`;
  ...
  return shell({
    ...
    subtitle: "Palworld tools — partner skill catalog (paldb.cc)",
...
function buildHomePage() {
  const skillCount = (partnerPaldb.skills || []).length;
  ...
  const featured = [
    ...
    featureCard({
      ...
      desc: "Partner skills from paldb.cc, our source of truth for game data.",
      meta: skillCount + " skills · paldb.cc",
      icon: "PS",
    }),
    featureCard({
      href: "base-tips.html",
    ...
    featureCard({
      ...
      meta: statusCount + " effects · in-game source",
      icon: "Fx",
    }),
    featureCard({
      empty: true,
      ...
      <div class="home-top-grid">
        ...
        <section class="panel">
          ...
          <ul class="stat-list">
            ...
            <li><span class="label">Partner skills (paldb)</span><span class="val"><a href="partner-skills.html">${skillCount}</a></span></li>
            <li><span class="label">Base work +1 boosters</span><span class="val"><a href="base-tips.html">${BASE_WORK_BOOSTERS.length}</a></span></li>
            <li><span class="label">Status effects (Survival Guide)</span><span class="val"><a href="status-effects.html">${statusCount}</a></span></li>
            <li><span class="label">Game data source</span><span class="val"><a href="https://paldb.cc" target="_blank" rel="noopener noreferrer">paldb.cc</a></span></li>
          </ul>
        </section>
      </div>
  ...
  return shell({
    ...
    subtitle: "Work suitability · partner skills · guides",
    activeNav: "home",
    body,
    showPromo: true,
  });
...
console.log("partner skills:", (partnerPaldb.skills || []).length);
